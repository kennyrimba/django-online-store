...
def product_computer(request, id):
    ...
    return render(
        request, "store/product/computer.html", {"data": product, "fullstar": fullstar}
    )
...
def product_smartphone(request, id):
    ...
    return render(
        request,
        "store/product/smartphone.html",
        {"data": product, "fullstar": fullstar},
    )
...
def product_headphone(request, id):
    ...
    fullstar = "★" * floor(product.stars)
    return render(
        request, "store/product/headphone.html", {"data": product, "fullstar": fullstar}
    )
...
def product_cloth(request, id):
    ...
    fullstar = "★" * floor(product.stars)
    return render(
        request, "store/product/cloth.html", {"data": product, "fullstar": fullstar}
    )
    ...
    if product_object:
        ...
        for product in product_object.values_list("title"):
            try:
                ...
                prod_link.append(
                    "/computer/product/"
                    + str(Computer.objects.get(title=product[0]).id)
                )
                prod_is_in_stock.append(
                    Computer.objects.get(title=product[0]).is_in_stock
                )
                prod_in_stocks.append(Computer.objects.get(title=product[0]).in_stocks)
                prod_is_recommend.append(
                    Computer.objects.get(title=product[0]).is_recommend
                )
            ...
            try:
                ...
                prod_link.append(
                    "/smartphone/product/"
                    + str(Smartphone.objects.get(title=product[0]).id)
                )
                prod_is_in_stock.append(
                    Smartphone.objects.get(title=product[0]).is_in_stock
                )
                prod_in_stocks.append(
                    Smartphone.objects.get(title=product[0]).in_stocks
                )
                prod_is_recommend.append(
                    Smartphone.objects.get(title=product[0]).is_recommend
                )
            ...
            try:
                ...
                prod_link.append(
                    "/headphone/product/"
                    + str(Headphone.objects.get(title=product[0]).id)
                )
                prod_is_in_stock.append(
                    Headphone.objects.get(title=product[0]).is_in_stock
                )
                prod_in_stocks.append(Headphone.objects.get(title=product[0]).in_stocks)
                prod_is_recommend.append(
                    Headphone.objects.get(title=product[0]).is_recommend
                )
            ...
            try:
                ...
                prod_link.append(
                    "/cloth/product/" + str(Cloth.objects.get(title=product[0]).id)
                )
                ...
                prod_is_recommend.append(
                    Cloth.objects.get(title=product[0]).is_recommend
                )
        ...
        for i in range(len(product_object)):
            ...
            prod_data.append(
                {
                    "title": prod_title[i],
                    "onsale": prod_onsale[i],
                    "ogprice": prod_og_price[i],
                    "price": prod_price[i],
                    "im": prod_img[i],
                    "instock": prod_is_in_stock[i],
                    "available": prod_in_stocks[i],
                    "link": prod_link[i],
                    "recommend": prod_is_recommend[i],
                    "star": fullstar,
                    "star_num": " (" + str(prod_star[i]) + ")",
                }
            )

    return render(
        request,
        "store/product_no_form.html",
        {"product_data": prod_data, "fullstar": fullstar},
    )
...
def OrderSummaryView(request):
    try:
        ...
        for i in range(len(prod_id_list)):
            ...
            if (
                "unisex" in prod.title
                or "Unisex" in prod.title
                or "Female" in prod.title
                or "Male" in prod.title
            ):
                prodlink = "/cloth/product/" + str(
                    Cloth.objects.get(title=prod.title).id
                )
            elif "Desktop" in prod.title or "Laptop" in prod.title:
                prodlink = "/computer/product/" + str(
                    Computer.objects.get(title=prod.title).id
                )
            elif (
                "Headphone" in prod.title
                or "Headset" in prod.title
                or "Earphone" in prod.title
            ):
                prodlink = "/headphone/product/" + str(
                    Headphone.objects.get(title=prod.title).id
                )
            else:
                prodlink = "/smartphone/product/" + str(
                    Smartphone.objects.get(title=prod.title).id
                )
            ...
            prod_list.append(
                {
                    ...
                    "image": prod.image,
                    "price": prod.price,
                    "totalprice": prod.price * quantity[i],
                    "quantity": quantity[i],
                }
            )
    ...
    return render(
        request,
        "store/order_summary.html",
        {
            "prodlist": prod_list,
            "total_item": total_item,
            "total_and_delivery_price": total_and_delivery_price,
        },
    )
...
def add_to_cart(request, title):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(prod_item=prod_item, ordered=False).exists():
            order_item, created = order.items.get_or_create(
                prod_item=prod_item, ordered=False
            )
            ...
            order_item = OrderProductItem.objects.create(
                prod_item=prod_item, user=request.user, ordered=False
            )
...
def remove_from_cart(request, title):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(prod_item__title=prod_item.title).exists():
            order_item = OrderProductItem.objects.filter(
                prod_item=prod_item, user=request.user, ordered=False
            )[0]
